...
Fig.2C.Accuracy.Thermoregulation.db <- ggplot(season.db, aes(x=Season, y=emmean, colour=Sex, group=Sex, shape = Sex)) +
  ...
  geom_line(aes(linetype=Sex)) +
  geom_point(aes(shape=Sex, color=Sex), size=3) +
  scale_shape_manual(values=c(19, 21), labels = c("db Female", "db Male")) +
  scale_color_manual(values=c("#999999", "black"), labels = c("db Female", "db Male")) +
...
survival_plot <- ggplot(dat, aes(x = Pmax*100, # back transform because MARK makes you put parameters in 0-1
